...
"""
批量运行蒸馏方法的训练脚本 - V2版本
基于参考实现重新整合CAT_KD、ICKD、MGD、SDD、DiffKD、MKD

修改说明:
- 根据experiments/1_comparison/reference中的参考实现重新集成CAT_KD、ICKD、MGD、SDD、DiffKD、MKD
- 确保这些方法的输入格式和计算逻辑与参考实现一致
- 默认仅训练6种基于参考实现的方法

包含的蒸馏方法:
基于参考实现的 6 种方法:
  1. CAT_KD - 通道对齐蒸馏 (参考实现)
  2. ICKD - 内部相关性蒸馏 (参考实现)
  3. VkD - 变分蒸馏 (参考实现)
  4. MGD - 掩码生成蒸馏 (参考实现)
  5. SDD - 结构解耦蒸馏 (参考实现)
...
DISTILL_METHODS_ALL = [
    ...
    "cc",          # 7. 相关性一致性
    "vid",         # 8. 变分信息蒸馏
    "cat_kd",      # 9. 通道对齐蒸馏 (参考实现)
    "ickd",        # 10. 内部相关性蒸馏 (参考实现)
    "vkd",         # 11. 变分蒸馏 (参考实现)
    "mgd",         # 12. 掩码生成蒸馏 (参考实现)
    "sdd",         # 13. 结构解耦蒸馏 (参考实现)
    "diffkd",      # 14. 扩散蒸馏 (参考实现)
...
# 仅训练基于参考实现的 6 种方法
DISTILL_METHODS = [
    "cat_kd",      # 1. CAT_KD - 通道对齐蒸馏 (参考实现)
    "ickd",        # 2. ICKD - 内部相关性蒸馏 (参考实现)
    "vkd",         # 3. VkD - 变分蒸馏 (参考实现)
    "mgd",         # 4. MGD - 掩码生成蒸馏 (参考实现)
    "sdd",         # 5. SDD - 结构解耦蒸馏 (参考实现)
    "diffkd",      # 6. DiffKD - 扩散蒸馏 (参考实现)
]
...
def setup_logger(save_dir: str, rank: int) -> logging.Logger:
    """设置日志记录器"""
    logger = logging.getLogger('comparison_distillation_v2')
...
# ========================================
# 训练器类（V2版本 - 根据参考实现重新整合）
# ========================================
class ComparisonDistillationTrainerV2:
    """对比蒸馏训练器 V2 - 基于参考实现"""
    def __init__(
        self,
        model: nn.Module,
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> dict:
        ...
        for batch_idx, (input_data, labels) in enumerate(train_loader):
            ...
            try:
                ...
                elif self.distill_method in ['at', 'nst', 'fsp', 'pkt', 'rkd', 'sp', 'cc', 'vid']:
                    # 特征蒸馏方法
                ...
                elif self.distill_method in ['cat_kd', 'ickd', 'uatr_kd', 'vkd']:
                    # 通道/相关性蒸馏方法（参考实现）
    ...
    def _compute_feature_distillation_loss(self, student_logits, stu_seq_logits,
        ...
        if output_cnn_features is not None:
            ...
            if self.distill_method == 'pkt':
                ...
                with torch.no_grad():
                    teacher_for_pkt = self.model.module.teacher_linear(t_h)
                soft_loss = self.criterion(student_for_pkt, teacher_for_pkt)
            elif self.distill_method == 'freekd':
                student_for_freekd = x_encoder.permute(0, 2, 1).unsqueeze(-1)
                teacher_for_freekd = t_h.permute(0, 2, 1).unsqueeze(-1)
    ...
    def _compute_special_distillation_loss(self, student_logits, stu_seq_logits,
        ...
        注意：DiffKD loss 内部包含所有需要的组件（trans, model, scheduler, noise_adapter等）
        """
        # 硬标签损失
        hard_loss = F.cross_entropy(student_logits, labels)
    ...
    def validate(self, val_loader: DataLoader) -> dict:
        ...
        try:
            ...
            self.logger.warning(f'Error computing AUC/F1: {e}')
            import traceback
...
def main_worker(rank: int, world_size: int, config: dict, gpu_ids: list):
    ...
    if rank == 0:
        logger.info('='*50)
        logger.info('Comparison Distillation Training V2 (基于参考实现)')
        logger.info('='*50)
    ...
    # 训练器
    trainer = ComparisonDistillationTrainerV2(
        model=model,
        criterion=criterion,
        optimizer=optimizer,
        scheduler=scheduler,
...
# ========================================
def main():
    parser = argparse.ArgumentParser(description='批量运行 6 种基于参考实现的蒸馏方法 (V2版本)')
    ...
    parser.add_argument('--method', type=str, default=None,
                        help='只训练特定的蒸馏方法，如 "cat_kd"')
    parser.add_argument('--skip_completed', action='store_true', default=True,
                        help='跳过已完成的训练（默认：True）')
    parser.add_argument('--all_methods', action='store_true', default=False,
                        help='训练所有21种方法（默认：False，仅训练6种参考实现的方法）')
    ...
    if args.all_methods:
        # 训练所有 21 种方法
        methods_to_train = DISTILL_METHODS_ALL
        training_mode = "所有 21 种方法"
    elif args.method:
        # 训练特定方法
        methods_to_train = [args.method]
        if args.method not in DISTILL_METHODS_ALL:
            raise ValueError(f"未知的蒸馏方法: {args.method}")
        training_mode = f"特定方法: {args.method}"
    else:
        # 默认：仅训练 6 种基于参考实现的方法
        methods_to_train = DISTILL_METHODS
        training_mode = "6 种基于参考实现的方法"

    # 打印开始信息
    print('='*80)
    print('批量蒸馏方法对比实验 V2 (基于参考实现)')
    print('='*80)
    ...
    for idx, method in enumerate(methods_to_train, 1):
        ...
        method_save_dir = original_base_save_dir.replace('/comparison_distillation',
                                                       f'/comparison_distillation_v2_{method}')
    ...
    csv_path = original_base_save_dir.replace('/comparison_distillation',
                                         '/comparison_distillation_v2_results.csv')
    import csv
