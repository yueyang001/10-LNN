...
class DRASP(nn.Module):
    ...
    def forward(self, x):
        ...
        # Segmentatio维度变换: (B, C, T_pad) -> (B, C, N_seg, Seg_Len) -> (B * N_seg, C, Seg_Len),Seg_Len每个片段的时间长度
...
class BiParallelCrossSliceCfC(nn.Module):
    ...
    def _process_stream(self, x, local_cfc, global_cfc):
        ...
        out_local = out_local.reshape(B, N, W, -1).view(B, L, -1)#L的值是固定的
        # Global Branch
        x_global = x.view(B, N, W, C).permute(0, 2, 1, 3).reshape(B * W, N, C) # (B, N, W, C)->(B,W, N, C)->(B*W,N,C)
...
class AudioCfC(nn.Module):
    def __init__(self, num_classes=5,  p_encoder=0.2, p_classifier=0.3):
        ...
        self.audio_encoder = nn.Sequential(
            ...
            nn.Conv1d(1, 32, kernel_size=7, stride=3, padding=3),# (B,1,48000)->(B,32,16000),32个卷积核
            nn.BatchNorm1d(32), nn.LeakyReLU(0.1, True), nn.AvgPool1d(2),# (B,32,16000)->(B,32,8000)
            nn.Conv1d(32, 64, kernel_size=5, stride=2, padding=2),# (B,32,8000)->(B,64,4000)
            nn.BatchNorm1d(64), nn.LeakyReLU(0.1, True), nn.AvgPool1d(3),# (B,64,4000)->(B,64,1334)
            nn.Conv1d(64, 64, kernel_size=3, stride=2, padding=1),# (B,64,1334)->(B,64,667)
            nn.BatchNorm1d(64), nn.LeakyReLU(0.1, True), nn.AvgPool1d(4),# (B,64,667)->(B,64,167)
            nn.Conv1d(64, 64, kernel_size=3, stride=2, padding=1),# (B,64,167)->(B,64,84)
            nn.BatchNorm1d(64), nn.LeakyReLU(0.1, True), nn.AvgPool1d(5),# (B,64,84)->(B,64,16)
            nn.Conv1d(64, 64, kernel_size=3, stride=1, padding=1),# (B,64,16)->(B,64,16)
    ...
    def forward(self, x):
        # x: (B, 1, 48000)
        x_encoder = self.audio_encoder(x) # -> (B,64,16) batch,features,time
        # print(f"编码器输出形状: {x_encoder.shape}") # -> (B, 64, 16)
        x_encoder = x_encoder.permute(0, 2, 1) # -> (B, 16, 64) batch,time,features
        x_encoder = self.encoder_dropout(x_encoder)
        seq_features, fl, fg, bl, bg = self.bi_parallel_cfc(x_encoder) # seq_features: (B,16,64) (B,time,features)
        pooled_features = self.drasp(seq_features.permute(0, 2, 1)) # (B,features,time)
