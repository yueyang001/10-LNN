...
class AudioDistillationModel(nn.Module):
    ...
    def __init__(
        ...
        # 添加教师特征投影层 (用于 PKT 等需要相同特征维度的方法)
        # 将教师特征从 512 维投影到 64 维 (与学生特征相同)
        self.teacher_linear = nn.Linear(512, cfc_hidden_size)  # 512 -> 64
        # 冻结 teacher_linear，因为它只是辅助投影层，不应该参与优化
        for param in self.teacher_linear.parameters():
            param.requires_grad = False
        
        # 添加学生特征投影层 (用于 CAT_KD 等需要将学生特征投影到教师特征维度的方法)
        # 将学生特征从 64 维投影到 512 维 (与教师特征相同)
        self.student_to_teacher_proj = nn.Linear(cfc_hidden_size, 512)  # 64 -> 512
        # 冻结 student_to_teacher_proj，因为它只是辅助投影层，不应该参与优化
        for param in self.student_to_teacher_proj.parameters():
            param.requires_grad = False
