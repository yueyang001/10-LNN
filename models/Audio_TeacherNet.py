...
class Audio_TeacherNet(torch.nn.Module):
    ...
    def forward(self, x, output_all_hidden_states=True):
        ...
        # 32, 149, 512 
        # cnn网络输出，Wav2Vec2 CNN feature extractor 的输出通道数
...
def build_Audio_TeacherNet(num_classes=5, checkpoint_path=None):
    ...
    if checkpoint_path is not None:
        ...
        # 检查checkpoint中的类别数
        if 'duration_linear.2.weight' in checkpoint:
            checkpoint_num_classes = checkpoint['duration_linear.2.weight'].shape[0]
            if checkpoint_num_classes != num_classes:
                print(f"Warning: Checkpoint has {checkpoint_num_classes} classes, but model has {num_classes} classes.")
                print(f"Loading checkpoint with mismatched categories using strict=False...")
                # 过滤掉分类层的权重
                checkpoint_filtered = {k: v for k, v in checkpoint.items()
                                     if not k.startswith('duration_linear.2.')}
                model.load_state_dict(checkpoint_filtered, strict=False)
                print(f"Loaded Audio_TeacherNet weights from {checkpoint_path} (excluding classifier layer)")
            else:
...
if __name__ == "__main__":
    ...
    print(f"output.shape: {output.shape}") # [4, 4] [batch,num_classes]
    print(f"CNN feat_ext.shape: {feat_ext.shape}") # [4, 149, 512] [batch,seq_len,hidden_size]
    print(f"feat_last.shape: {feat_last.shape}") # [4, 149, 1024] [batch,seq_len,hidden_size]
    if all_hidden_states is not None:
        print(f"所有隐藏状态形状: {all_hidden_states.shape}")  # torch.Size([13, 4, 149, 1024]) [num_layers + 1, B, seq_len, hidden_size]
