"""
DeepShip K-Fold 50折叠交叉验证 + 纯学生网络(LNN)训练 完整集成脚本
...
使用方式:
    ...
    或
    python kfold_LNN_deepship_integration.py --all  # 运行所有50个Fold
"""
...
class LNNDeepShipKFoldTrainer:
    ...
    def train_fold(self, fold_idx, config, gpus="4,5,6,7"):
        ...
        """
        print(f"\n{'='*80}")
        print(f"DeepShip LNN K-Fold Training - Fold {fold_idx}/49")
        print(f"{'='*80}")
    ...
    def _real_train(self, fold_idx, save_dir, gpus="4,5,6,7"):
        ...
        try:
            ...
            cmd = (
                f"python train_LNN_deepship.py "
                f"--config {tmp_config} "
                f"--gpus {gpus} "
                f"2>&1 | tee {log_file}"
            )
    ...
    def _generate_group_report(self, group_name, group_results, start_fold, end_fold):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write(f"DeepShip LNN K-Fold {group_name} (Fold {start_fold}-{end_fold-1}) 10折叠交叉验证 - 训练总结\n")
    ...
    def generate_summary_report(self, results):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write("DeepShip LNN K-Fold 50折叠交叉验证 - 训练总结报告\n")
            f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 80 + "\n\n")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="DeepShip LNN K-Fold交叉验证训练")
    ...
    # 创建训练器
    trainer = LNNDeepShipKFoldTrainer(
...
if __name__ == "__main__":
    # 如果没有命令行参数，运行演示
    if len(sys.argv) == 1:
        print("📌 DeepShip LNN K-Fold交叉验证 + 纯学生网络训练演示\n")
        print("使用示例:")
        print("  python kfold_LNN_deepship_integration.py --fold 0          # 训练Fold 0")
        print("  python kfold_LNN_deepship_integration.py --all             # 训练所有Fold")
        print("  python kfold_LNN_deepship_integration.py --all --gpus 0,1  # 使用指定GPU训练\n")
