...
class MultiDeepShipKFoldTrainer:
    """DeepShip多组K-Fold训练管理器"""
    ...
    def __init__(self, data_dir, output_base_dir="results", num_splits=3, gpus="4,5,6,7"):
        ...
        self.splits_dir = os.path.join(output_base_dir, "kfold_splits_multi_deepship")
        self.results_dir = os.path.join(output_base_dir, "kfold_cv_deepship_multi")
    ...
    def generate_multiple_splits(self):
        """生成多组K-Fold划分"""
        print("\n" + "="*80)
        print("📊 第1步: 生成多组K-Fold划分 (DeepShip)")
        print("="*80)
    ...
    def train_split(self, split_id, split_config):
        ...
        print(f"\n{'='*80}")
        print(f"【第 {split_id+1}/{len(self.split_configs)} 组】训练: {split_name} (DeepShip)")
        print(f"{'='*80}")
        ...
        cmd = (
            f"python kfold_deepship_integration.py "
            f"--all "
            f"--gpus {self.gpus} "
    ...
    def print_summary(self):
        ...
        print("  git commit -m 'Add DeepShip multi-split K-Fold results'")


def main():
    """主函数"""
    parser = argparse.ArgumentParser(
        description="DeepShip 多组K-Fold划分完整训练流程",
...
示例用法:
  ...
  python kfold_multi_train_deepship.py --compare \\
    ...
    # 创建训练器
    trainer = MultiDeepShipKFoldTrainer(
