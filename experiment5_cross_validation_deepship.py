...
search_space = {
    ...
    # ===== 固定训练参数（根据 grid_search_A.py 设置 DeepShip 最优值）=====
    "training.num_epochs": [200],
    "training.weight_decay": [8e-5],    # DeepShip 参数
    "training.batch_size": [16],        # DeepShip 参数
    "training.lr": [0.0004],            # DeepShip 参数
 
    # ===== 蒸馏类型 =====
    # 探索过拟合通常针对最终模型 MTSKD，如需测试 MemKD 请修改此处
    "distillation.distill_type": ["MTSKD"], 
 
    # ===== 其他固定参数（根据 grid_search_A.py 设置 DeepShip 最优值）=====
    "model.teacher.freeze": [True],
    "distillation.USE_DYNAMIC_DISTILL_WEIGHT": [False],
    "model.student.p_encoder": [0.1],      # DeepShip 参数
    "model.student.p_classifier": [0.35],  # DeepShip 参数
}
...
print(f"\n🔥 DeepShip 十倍交叉验证实验总数: {len(all_combinations)} 组")
 
########################################
# 目录设置
########################################
root_save_dir = "checkpoints/cv_deepship"
os.makedirs(root_save_dir, exist_ok=True)
 
results_save_dir = "results/cv_deepship"
...
for idx, combo in enumerate(all_combinations):
    ...
    print(f"\n===== DeepShip 十倍交叉验证实验 {idx+1}/{len(all_combinations)} =====")
    ...
    # 启动训练
    ########################################
    # 注意：这里修改为调用 deepship 的训练脚本，GPU 设置为 0-7 (参考 grid_search_A.py)
