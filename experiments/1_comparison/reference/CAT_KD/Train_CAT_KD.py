...
if __name__ == '__main__':
    ...
    snapshot_path = './CheckPoints/DeepShip_CAT_KD/'
    ...
    for cross in range(args.num_train_test):
        ...
        student.train()
        model = CAT_KD(student=student, teacher=teacher).cuda()
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for index, (mel, label) in enumerate(train_loader):
                ...
                s_out, CAM_Student, CAM_Teacher = model(mel)

                loss_task = criterion_task(s_out, label)
                loss_cat = CAT_loss(CAM_Student, CAM_Teacher, 4, True)
                loss = loss_task + loss_cat
