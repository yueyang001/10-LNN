...
from ResNet_VID import resnet18
from Student_VID import StudentNet
...
if __name__ == '__main__':
    ...
    snapshot_path = './Checkpoints_EarShip_VID/'
    ...
    criterion_VID_1 = VIDLoss(64, 32, 64).cuda()
    criterion_VID_2 = VIDLoss(128, 64, 128).cuda()
    criterion_VID_3 = VIDLoss(256, 128, 256).cuda()
    optimizer = optim.AdamW(model_S.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2)
    writer = SummaryWriter(snapshot_path + 'log')

    best_model = copy.deepcopy(model_S.state_dict())
    best_acc = 0
    best_epoch = 0
        ...
        for phase in ['train', 'test']:
            ...
            for i, (inputs, label) in enumerate(dataset[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    teacher_output, teacher_VID = model_T(inputs)
                    student_output, student_VID = model_S(inputs)
                    ...
                    loss_VID_1 = criterion_VID_1(student_VID[0], teacher_VID[0])
                    loss_VID_2 = criterion_VID_2(student_VID[1], teacher_VID[1])
                    loss_VID_3 = criterion_VID_3(student_VID[2], teacher_VID[2])
                    loss = loss_task + loss_kl + loss_VID_1 + loss_VID_2 + loss_VID_3
    ...
    model_S.load_state_dict(best_model)
    torch.save(model_S.state_dict(), snapshot_path + '/best_acc.pth')

    final_model = copy.deepcopy(model_S.state_dict())
    model_S.load_state_dict(final_model)
    torch.save(model_S.state_dict(), snapshot_path + '/final_epoch.pth')
