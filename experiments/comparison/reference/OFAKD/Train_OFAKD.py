...
if __name__ == '__main__':
    ...
    snapshot_path = './CheckPoints/DeepShip_OFAKD/'
    ...
    for cross in range(args.num_train_test):
        ...
        student.train()
        model = OFA(teacher=teacher, student=student, num_classes=4).cuda()
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for index, (mel, label) in enumerate(train_loader):
                ...
                s_out, loss_ofa = model(mel, label)
                loss_task = criterion_task(s_out, label)
                loss = loss_task + loss_ofa
