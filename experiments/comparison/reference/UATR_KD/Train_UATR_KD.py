...
def kd_loss(logit_student, logit_teacher, temperature):
    log_pred_student = F.log_softmax(logit_student / temperature, dim=1)
    pred_teacher = F.softmax(logit_teacher / temperature, dim=1)
...
if __name__ == '__main__':
    ...
    snapshot_path = './CheckPoints/DeepShip_UATR_KD/'
    ...
    for cross in range(args.num_train_test):
        ...
        teacher.load_state_dict(torch.load('./CheckPoints/DeepShip_Teacher/' + str(cross + 1) + '/best_acc.pth'))
        teacher.eval()
        student = CNNStudent().cuda()
        # student.load_state_dict(torch.load('./CheckPoints/DeepShip_Student/' + str(cross + 1) + '/best_acc.pth'))
        student.train()
        model = UATR_KD(student=student, teacher=teacher).cuda()
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for index, (mel, label) in enumerate(train_loader):
                ...
                s_out, t_out, s_fea, t_fea = model(mel)

                loss_task = criterion_task(s_out, label)
                loss_kd_logit = kd_loss(s_out, t_out, 3)
                loss_kd_fea = kd_loss(s_fea, t_fea, 3)
                loss = loss_task + loss_kd_logit + loss_kd_fea
