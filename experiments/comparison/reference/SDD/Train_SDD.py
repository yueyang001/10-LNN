...
if __name__ == '__main__':
    ...
    snapshot_path = './CheckPoints/DeepShip_SDD/'
    ...
    for cross in range(args.num_train_test):
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for index, (mel, label) in enumerate(train_loader):
                ...
                s_out, s_patch = model(mel)
                with torch.no_grad():
                    t_out, t_patch = teacher(mel)

                loss_task = criterion_task(s_out, label)
                loss_sdd = sdd_kd_loss(s_patch, t_patch, 3, label)
                loss = loss_task + loss_sdd
