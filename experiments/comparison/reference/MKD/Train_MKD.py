...
from CNNStudent1 import CNNStudent
...
if __name__ == '__main__':
    ...
    snapshot_path = './CheckPoints/DeepShip_MKD1/'
    ...
    for cross in range(args.num_train_test):
        ...
        student.train()
        model = MKD(teacher=teacher, student=student, mask_ratio=0.1, depth=4).cuda()
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            for index, (mel, label) in enumerate(train_loader):
                ...
                s_out, t_out, s_mask, t_mask = model(mel)

                loss_task = criterion_task(s_out, label)
                loss_mask = torch.tensor(0.).cuda()
                for i in range(3):
                    loss_mask += criterion_feat(s_mask[i], t_mask[i])
                loss = loss_task + loss_mask
